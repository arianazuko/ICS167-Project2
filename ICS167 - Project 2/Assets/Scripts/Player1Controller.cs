...
public class Player1Controller : MonoBehaviour
{
    ...
    void Move()
    {
        ...
        //continuous spinning
        if (Input.GetAxisRaw("Vertical") != 0 && !rigidSpinning)
        {
            this.transform.Rotate(0, 0, spinSpeed * Input.GetAxisRaw("Vertical"));
        }

        //rigid spinning
        if (Input.GetKeyDown(KeyCode.W) && rigidSpinning)
        {
            this.transform.Rotate(0, 0, 45);
        }
        if (Input.GetKeyDown(KeyCode.S) && rigidSpinning)
        {
            this.transform.Rotate(0, 0, -45);
        }
